...
-- This script inserts test data with proper UUID format
...
-- Get the bread type IDs we just created
WITH bread_type_ids AS (
  SELECT id, name FROM bread_types WHERE name IN ('White Bread', 'Whole Wheat', 'Sourdough', 'Rye Bread', 'Baguette')
)
-- Insert sample batches using the actual bread type IDs
...
ON CONFLICT (bread_type_id, batch_number) DO NOTHING;
