...
CREATE TABLE IF NOT EXISTS public.available_stock (
  ...
  -- Ensure unique bread type per stock entry
...
-- Function to automatically update available stock
...
BEGIN
  ...
  IF TG_TABLE_NAME = 'production_logs' THEN
    ...
    VALUES (NEW.bread_type_id, (SELECT name FROM public.bread_types WHERE id = NEW.bread_type_id), NEW.quantity, (SELECT unit_price FROM public.bread_types WHERE id = NEW.bread_type_id))
    ON CONFLICT (bread_type_id) 
    DO UPDATE SET 
      quantity = available_stock.quantity + NEW.quantity,
      unit_price = (SELECT unit_price FROM public.bread_types WHERE id = NEW.bread_type_id),
      last_updated = now(),
      updated_at = now();
...
-- Allow managers and owners to update stock
...
-- Allow managers and owners to insert stock
...
SELECT 
  bt.id,
  bt.name,
  COALESCE(SUM(pl.quantity), 0) - COALESCE(SUM(sl.quantity), 0) as available_quantity,
  bt.unit_price
