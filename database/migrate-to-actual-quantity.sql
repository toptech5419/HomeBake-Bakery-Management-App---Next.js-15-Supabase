...
-- This script will:
-- 1. Rename target_quantity to actual_quantity in both batches and all_batches tables
-- 2. Update the database function to use actual_quantity
-- 3. Update all related constraints and indexes

-- Step 1: Backup existing data (optional but recommended)
-- CREATE TABLE batches_backup AS SELECT * FROM batches;
-- CREATE TABLE all_batches_backup AS SELECT * FROM all_batches;

-- Step 2: Update the database function to use actual_quantity
...
-- Step 3: Update the API route to use actual_quantity
...
-- Step 5: Update all_batches table to use actual_quantity
-- Note: This assumes the all_batches table has the same structure
-- If you need to migrate existing data, you would run:
-- UPDATE all_batches SET actual_quantity = target_quantity WHERE actual_quantity IS NULL;
-- ALTER TABLE all_batches DROP COLUMN IF EXISTS target_quantity;
