...
export function MobileNotificationProvider({ children }: { children: React.ReactNode }) {
  ...
  const showNotification = useCallback((notification: Omit<Notification, 'id'>) => {
    ...
    if (newNotification.soundEnabled && typeof window !== 'undefined' && 'Audio' in window) {
      try {
  ...
  return (
    <NotificationContext.Provider value={{ showNotification, clearNotification, clearAll }}>
      ...
      <div 
        ...
        style={{
          bottom: typeof window !== 'undefined' && window.innerWidth < 640 ? `${safeAreaInsets.bottom}px` : 'auto',
          left: `${safeAreaInsets.left}px`,
          right: `${safeAreaInsets.right}px`,
          top: typeof window !== 'undefined' && window.innerWidth >= 640 ? `${safeAreaInsets.top}px` : 'auto',
        }}
      >
          ...
          <AnimatePresence mode="popLayout">
            {notifications.map((notification, index) => (
              <motion.div
                ...
                initial={{ 
                  opacity: 0, 
                  y: typeof window !== 'undefined' && window.innerWidth >= 640 ? -50 : 50,
                  scale: 0.95,
                  filter: 'blur(4px)'
                }}
                animate={{ 
                ...
                exit={{ 
                  opacity: 0, 
                  y: typeof window !== 'undefined' && window.innerWidth >= 640 ? -30 : 30,
                  scale: 0.9,
                  filter: 'blur(2px)',
                  transition: {
                    duration: 0.2
